...
public class MarkDaoImpl implements MarkDao{
    ...
    public boolean inserMarkOfStudent(Mark mark) {
            String sql = "INSERT INTO mark (mark1, mark2, mark3, course_idcourse, student_idstudent) "
                    + "VALUES (?, ?, ?, ?, ?);";
        try(
    ...
    public boolean updateMarkOfStudent(Mark mark) {
        String sql = "UPDATE mark SET mark1 = ?, mark2 = ?, mark3 = ?  WHERE course_idcourse = ? and student_idstudent = ?;";
        try(
                Connection con = com.appdesktop.StudentManagement.DBHelpers.DBHelper.openConnection();
                PreparedStatement pstmt = con.prepareStatement(sql);
           ) {
    ...
    public Mark getMarkOfStudent(String idCourse, String idStudent) {
        String sql = "SELECT  mark.mark1, mark.mark2, mark.mark3 From mark  " + 
        " where mark.student_idstudent = ? and mark.course_idcourse = ?;";
        try(
                Connection con = com.appdesktop.StudentManagement.DBHelpers.DBHelper.openConnection();
